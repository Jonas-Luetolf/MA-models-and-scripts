...
MODELS = glob.glob(os.path.join("./dieguten/", '*.hdf5'))
print(len(XTEST_180))
for model in MODELS:
    ...
    print(f"Correct predictions percentage (>50%): {round(correct/len(y)*100, 2)}%")
    print(f"Correct predictions percentage (>70%): {round(correct_70/len(y)*100, 2)}%")
    ...
    norm[norm == 0] = 1
    norm_vx = 3 * wrong["v_x"] / norm
    norm_vy = 3 * wrong["v_y"] / norm
    ...
    plt.savefig(f"{"-".join(map(str, structure))}-analyse.png")
    plt.savefig(f"{"-".join(map(str, structure))}-analyse.pgf")
    plt.show()
